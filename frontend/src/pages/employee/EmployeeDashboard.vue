...
// Stats data (You can also fetch this from API if available)
const stats = ref([
  {
    title: 'Pending Tasks',
    value: 0,
...
const productivityData = ref({ labels: [], data: [] });
...
async function fetchTasks() {
  try {
    ...
    throw new Error('Failed to fetch tasks: ' + (err.response?.data?.message || err.message));
  }
}
...
async function fetchEvents() {
  try {
    ...
    throw new Error('Failed to fetch events: ' + (err.response?.data?.message || err.message));
  }
}
...
async function fetchActivities() {
  try {
    ...
    teamActivities.value = response.data;
  } catch (err) {
    throw new Error('Failed to fetch activities: ' + (err.response?.data?.message || err.message));
  }
}
...
async function fetchProductivityData() {
  try {
    ...
    throw new Error('Failed to fetch productivity data: ' + (err.response?.data?.message || err.message));
  }
}

// Optionally, fetch stats if available from API
async function fetchStats() {
  try {
    const response = await axios.get(`${API_BASE_URL}/stats`);
    // Assuming API returns stats in the same structure as your stats ref
    if (Array.isArray(response.data)) {
      stats.value = response.data;
    }
  } catch (err) {
    // If stats API not available, you can skip or handle differently
    console.warn('Stats API not available or failed to load');
  }
}
...
function completeTask(taskId) {
  ...
  if (task) {
    task.completed = true;
    task.overdue = false;
    // Optionally sync with backend here (PUT/PATCH)
  }
}
...
function handleTaskSubmit(taskData) {
  if (taskData.id) {
    ...
    // Optionally sync update with backend here (PUT/PATCH)
  } else {
    ...
    // Optionally sync create with backend here (POST)
  }
  closeTaskModal();
}
